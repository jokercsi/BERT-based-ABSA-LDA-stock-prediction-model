...
# pathlib의 기본 아이디어는 파일시스템 경로를 단순한 문자열이 아니라 객체로 다루자는 것입니다.
...
def parser_args():
    ...
    # Dateset 지정
    parser.add_argument("-n", "--news", default="./../data/text")
    # OUTPUT 지정
    parser.add_argument("-o", "--output", default="./../data/output_morphological.csv")
...
def Morphological(article, stopwords):
    ...
    headline = [WordNetLemmatizer().lemmatize(word, pos='v') for word in headline]
    # headline = [word for word in headline if len(word) > 2]
